/**
 * Everything about self-hosting Monaco except *where the workers come from*.
 *
 * The themes, the TypeScript language-service configuration and the
 * registration of the real editor implementations are the same wherever Monaco
 * runs. How its four web workers are delivered is not, and that one decision
 * costs about nine megabytes:
 *
 *   • A consumer inside a VS Code webview cannot fetch a worker script over
 *     the network — the CSP forbids it — so the workers have to be inlined
 *     into the bundle as blobs. That is what `monaco-setup.ts` does, and it is
 *     the published contract.
 *
 *   • A page on the open web can just fetch them, and should: inlining puts
 *     the whole TypeScript compiler, base64-encoded, into the chunk that has
 *     to arrive before anything renders. That is what `showcase/monacoSetup.ts`
 *     does.
 *
 * So the workers live in those two files and everything else lives here.
 * Nothing in this module reads or writes `MonacoEnvironment`; both wrappers set
 * it before importing this, and Monaco only reads it when the first editor
 * mounts.
 *
 * This file is not a public entry point. Consumers import
 * `@salilvnair/dui/monaco-setup`, whose behaviour is unchanged.
 */
